...
///////////// Here is here party comes in /////////////////
class MacroCommand : public ICommand
{
public:
    MacroCommand(const std::vector<ICommand*>& commands)
        : m_commands(commands)
    {
    }
...
int main()
{
    ...
    // Party on
    MacroCommand partyOn {{&garageDoorUpCommand, &ligthOnCommand, &sterioOnCommand}};

    // Party off
    MacroCommand partOff {{&garageDoorDownCommand, &lightOffCommand, &sterioOffCommand}};

    RemoteControl rc;

    rc.setCommand(0, &partyOn, &partOff);
    std::cout << rc << "\n";
    rc.onButtonPushed(0);
